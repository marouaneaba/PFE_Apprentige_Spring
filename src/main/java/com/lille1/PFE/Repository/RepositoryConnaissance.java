...
public interface RepositoryConnaissance extends CrudRepository<Connaissance, Long>{
	...
	public int updateConnaissanceById(@Param("connaissanceID")Long id,
						@Param("name")String nom,@Param("ordre")int ordre);
	...
	//@Query("select c from Connaissance c  where c.id_ExEtu != (select ec.id_ExEtu from Etudiant e JOIN e.connaissances ec)")
